                      ...
                      >
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`text-[9px] px-1 py-0.5 rounded flex items-center gap-0.5 ${
                            responseIndicator.color === 'green' ? 'bg-green-500/20 text-green-400' :
      ...
      <div className={`grid gap-4 ${viewMode === 'kanban' ? 'lg:hidden' : ''}`}>
        {filteredLeads.map((lead) => {
          ...
          const leadFollowUpCount = leadFollowUps.length;
            ...
            // Count pending scheduled tasks for this lead
            const leadScheduledTasks = scheduledTasks.filter(
              (t: { leadId: string; completed: boolean }) => t.leadId === lead.id && !t.completed
            );
            const scheduledTaskCount = leadScheduledTasks.length;
                  ...
                  <div className="flex-1">
                     <div className="flex items-center gap-3 mb-1 flex-wrap">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded border flex items-center gap-1 ${
                          lastResponse === 'POSITIVA' ? 'bg-green-500/20 text-green-400 border-green-500/30' :
                        ...
                        {scheduledTaskCount > 0 && (
                          <span className="text-[10px] px-2 py-0.5 rounded border flex items-center gap-1 bg-yellow-500/20 text-yellow-400 border-yellow-500/30 animate-pulse">
                            <CalendarClock size={10} />
                            {scheduledTaskCount} tarea{scheduledTaskCount > 1 ? 's' : ''}
                          </span>
                        )}
                     </div>
